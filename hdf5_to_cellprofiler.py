...
def main():
    ...
    ## get the indices of the hdf5 file this script will work on
    start, end = chunk_indices(input, chunks, rank)

    # read files and write
    with h5py.File(input, 'r') as f:
        keys =  list(f.keys())
        # subset the label keys
        ...
        # assume image dataset is the only one that is not one dimensional and get image data
    ...
    # move the cellprofiler output to the output
    os.rename("% s/nuclei.txt" % temp_path, "% s/nuclei% s% s.txt" % (output, str(name), str(rank)))
    os.rename("% s/file_list.txt" % temp_path, "% s/file_list% s% s.txt" % (output, str(name), str(rank)))
    os.rename("% s/Image.txt" % temp_path, "% s/Image% s% s.txt" % (output, str(name), str(rank)))
